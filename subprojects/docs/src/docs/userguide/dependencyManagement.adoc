...
//An example of how to do this in the build script would be nice here
//dependencies {
//    implementation('mygroup:mymodule:0.0.1') {
//       changing = true
//    }
//}
...
// It is probably ok to word it like this as this is an introduction, but it is also a bit missleading. You can also have dependencies with metadata in a folder on a shared drive or in version control. You do not nedd to "buy" a product for that. Any file system can serve as a repo with metadata. But how you do that is/should be described in the section about repositories.
// Maybe we could just add an additional sentence here with a link to that section. Also, that section should include an example of a `file://` based repository (atm it only says "Gradle supports other protocols than http or https as part of the custom URL e.g. file..."). I think defining "real" repositories with metadata on any server (e.g. shared drive) is an interesting feature for many users.
...
+++++
// exchange `compile` for `implementation` in the image
